...
/*
velocity cohesion(int const N, double const c, position const& CM, boid const&
b)
{
...
velocity cohesion(std::vector<boid> const& v, int const N, double const c,
                  boid const& b, double d)
{
  position init{0., 0.};
  position null_sum{0.0, 0.0};
  position cm = (std::accumulate(v.begin(), v.end(), init,
                                 [b, d, null_sum](position res, boid a) {
                                   return distance(b.pb, a.pb) < d
                                            ? res + a.pb
                                            : res + null_sum;
                                 })
                 - b.pb)
...
velocity alignment(double const a, boid const& b, std::vector<boid> const& v,
                   int const N, double d)
{
  velocity init{0.0, 0.0};
  velocity null_sum{0.0, 0.0};
...
position random_position_generator(const double min_position,
...
std::vector<boid> boids_generator(int const N, const double min_position,
                                  const double max_position,
                                  const double velocity_generation)
{
  std::vector<boid> v;
...
velocity wall_repulsion(boid boid, const double min_position,
                        const double max_position)
{
  auto boid1 = boid;
...
std::vector<boid> update_boids(std::vector<boid>& v, double s, double d,
                               double ds, double a, double c, int N,
                               const double min_position,
                               const double max_position,
                               const double max_velocity,
                               const double frame_rate_limit)
{
  ...
  for (auto& boid : v) {
    if (boid.pb.x > max_position || boid.pb.x < min_position
      ...
      auto boid1 = boid;
      int null   = 0;
      long int count  = std::count_if(
          v.begin(), v.end(), [d, boid1, null](class boid boid_puff) {
            return distance(boid1.pb, boid_puff.pb) < d ? null + 1 : null + 0;
          });
      int Nridotto = count;
      boid.vb += separation(s, ds, boid, v1) + alignment(a, boid, v1, Nridotto, d)
               + cohesion(v1, Nridotto, c, boid, d);
